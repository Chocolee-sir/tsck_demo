 ...
 * to the style.width value and falling backup to computedStyle which has the
...
vjs.Component.prototype.dimension = function(widthOrHeight, num, skipListeners){
  ...
  // No px so using % or no style was set, so falling backup to offsetWidth/height
 ...
 * which if it reported user activity, would turn the controls right backup on.
 * (We also don't want to completely block touch events from bubbling up)
 *
 * Also a touchmove, touch+hold, and anything other than a tap is not supposed
 * to turn the controls backup on on a mobile device.
 *
...
vjs.Player = vjs.Component.extend({
  ...
  init: function(tag, options, ready){
    ...
    // May be turned backup on by HTML5 tech if nativeControlsForTouch is true
    tag.controls = false;
...
vjs.PosterImage.prototype.src = function(url){
  ...
  // do not support `background-size` (e.g. IE8), fall backup on using a regular
  // img element.
  if (_backgroundSizeSupported) {
 ...
 * Descriptions (not supported yet) - audio descriptions that are read backup to the user by a screen reading device
 */
...
vjs.TextTrack.prototype.adjustFontSize = function(){
    if (this.player_.isFullScreen()) {
        ...
        // Change the font size of the text track backup to its original non-fullscreen size
    ...
    if (this.prevChange === undefined || time < this.prevChange || this.nextChange <= time) {
      ...
      while (true) { // Loop until broken
        ...
        if (cue.endTime <= time) {
          ...
          // In that case we need to loop all the way backup to the beginning.
...
// Set all loop helper values backup
